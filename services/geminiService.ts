...
export const generateImage = async (
    ...
    if (imageModel === 'imagen-4.0-generate-001') {
        ...
        try {
            ...
            if (error instanceof Error && (error.message.includes("billed users") || error.message.includes("INVALID_ARGUMENT"))) {
                 throw new Error("Imagen API may require a billed account. Try selecting 'Gemini Flash Image' or check your billing status.");
            }
            throw new Error("AI image generation failed with Imagen. Please check your API key and console for details.");
        }
    } else if (imageModel === 'gemini-2.5-flash-image') {
        const imagePrompt = `A high-quality, ${style} featured image for a blog article. The subject of the image should be visually stunning, clean, professional, and directly related to the article title: "${title}". The image should not contain any text.`;
        ...
        try {
            ...
            throw new Error("No image was generated by Gemini Flash Image.");
        } catch (error) {
            ...
            if (error instanceof Error && error.message.includes("RESOURCE_EXHAUSTED")) {
                throw new Error("Image generation failed due to API quota limits. This model may require a billed Google AI account. Please check your plan and billing details.");
            }
            throw new Error("AI image generation failed with Gemini Flash Image. Please check your API key and console for details.");
        }
    } else {
